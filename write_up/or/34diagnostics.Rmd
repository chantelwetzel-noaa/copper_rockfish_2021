...
Estimating recruitment deviations resulted in large changes in estimated stock size and status relative to the base model. The sensitivity that estimated recruitment deviations for all model years estimated a lower initial stock size relative to the base model (14.1 vs `r round(model$derived_quants[model$derived_quants$Label == 'SSB_1927','Value'], 1)` millions of eggs) and the estimated stock status (30 percent) was below the management target (Table \ref{tab:sensitivities-1} and Figures \ref{fig:sens-ssb-1} and \ref{fig:sens-depl-1}). This sensitivity estimated a multiple large recruitments in the late-1990s with only a few years after 2000 having above average recruitment (Figure \ref{fig:sens-recdev-1}). The two other sensitivities exploring the estimation of recruitment deviations (Rec. Devs. Fix Comm. and Rec. Devs. 1927-2000) each estimated stock scale and statuses that were intermediate to the all recruitment deviations sensitivity (Rec. Devs.) and the base model.
...
Likelihood profiles were conducted for $R_0$, steepness, female natural mortality, female $L_{\infty}$, female growth coefficient ($k$), female coefficient of variotion for older fish ($CV_2$), and the peak of the commercial selectivity  separately. These likelihood profiles were conducted by fixing the parameter at specific values and estimated the remaining parameters based on the fixed parameter value.
...
A profile across a range of female $L_{\infty}$ values was also conducted (Figure \ref{fig:linf-profile}).  The negative log-likelihood showed support for values between approximately 46.5 and 48.5 cm. The $L_{\infty}$ value for female fish in the model was fixed at `r model$parameters[model$parameters$Label == "L_at_Amax_Fem_GP_1","Value"]`. The stock scale and status is quite variable across alternative $L_{\infty}$ values where assuming lower values resulted in sharp increases in stock scale and status (Figures \ref{fig:linf-ssb} and \ref{fig:linf-depl}).
