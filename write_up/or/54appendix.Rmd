...
```{r, results = 'asis'}
data = read.csv("//nwcfile/FRAM/Assessments/CurrentAssessments/DataModerate_2021/copper_rockfish/write_up/or/table_1_sample_info.csv")

col_names = c("Site", "Area", "Years Sampled", "Total Samples")
table_format(x = data,
             caption = "Sites and areas sampled by the Mrine Reserve Program hook and line survey.",
             label = "table-1",
             align = 'l',
             custom_width = TRUE,
             col_to_adjust = 1:4,
...
```{r, results = 'asis'}
data = read.csv("//nwcfile/FRAM/Assessments/CurrentAssessments/DataModerate_2021/copper_rockfish/write_up/or/table_2_marine_hkl.csv")
...
```{r, results = 'asis'}
loc = "//nwcfile/FRAM/Assessments/CurrentAssessments/DataModerate_2021/copper_rockfish/write_up/or"
add_figure(
...
```{r, results = 'asis'}
data = read.csv("//nwcfile/FRAM/Assessments/CurrentAssessments/DataModerate_2021/copper_rockfish/write_up/or/table_3_data.csv")
...
table_format(x = data,
             caption = "SSummary of sampling effort by year and site combined with the positive observations of copper rockfish.",
             label = "table-3",
             align = 'l',
...
Additional filtering may not be necessary, as the filtering for "active" cells has already likely removed any unsuitable sampling units, based on habitat, depth and local knowledge.  Based on the annual proportion of positive cell-days and the relative rarity of `r spp` encounters, there are probably not enough data to move forward with a timeseries at a coastwide level.  However, Redfish Rocks has been sampled in each year from 2011 - 2019, except for 2018. Though sample size is extremely limited, CPUE at this site shows an increasing trend since 2011 for `r spp`.
...
caption = "Copper rockfish CPUE calculated at Redfish Rocks based on postive values only",
label = 'fig-4')
```
