...
bookdown::render_book("00a.Rmd", clean = FALSE, output_dir = getwd())
```
This will allow users to identify errors quickly.  If the document rendered correctly the user should see that "Output created: _main.pdf" followed by miscallaneous other warnings in the R terminal.  
...
This will create a new model output file which will replace the old file and be used to update the document to the new model the next time the document is rendered. 
