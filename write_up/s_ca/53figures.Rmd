...
caption = "Map of management area and assessments areas for copper rockfish with the assessment area south of Point Conception shown in red",
label = 'map')
```
...
caption = "NWFSC Hook and Line survey sample sites inside and outside the CCA and sites with observations of copper rockfish",
label = 'hkl-site-ob')
```
...
caption = "Fraction female by length across all available data sources where the size of red circles are based on the number of observations by length where larger circles indicate more observations",
label = 'len-sex-ratio')
```
...
caption = "Fraction female by age across all available data sources where the size of red circles are based on the number of observations by age where larger circles indicate more observations",
label = 'age-sex-ratio')
```
...
nm = paste0(basename(mod_loc), "_final_1_compare2_spawnbio_uncertainty.png")
add_figure(
...
```{r, echo=FALSE, results='asis'}
nm = paste0(basename(mod_loc), "_final_1_compare4_Bratio_uncertainty.png")
add_figure(
filein = file.path(sens_wd, nm),
...
```{r, echo=FALSE, results='asis'}
nm = paste0(basename(mod_loc), "_final_2_compare2_spawnbio_uncertainty.png")
add_figure(
filein = file.path(sens_wd, nm),
caption = "Change in estimated spawning output by sensitivity",
label = 'sens-ssb-2')
...
```{r, echo=FALSE, results='asis'}
nm = paste0(basename(mod_loc), "_final_2_compare4_Bratio_uncertainty.png")
add_figure(
filein = file.path(sens_wd, nm),
caption = "Change in estimated fraction unfished by sensitivity",
label = 'sens-depl-2')
