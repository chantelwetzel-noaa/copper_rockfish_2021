...
The length-at-age was estimated for male and female `r spp` using a mixture of data collected from fishery-dependent data sources off the coast of Oregon and Washington that were collected from 1998-2019 (Table \ref{tab:len-at-age-samps}) and length-at-age data from larger `r spp` captured by the `r hkl` and the \Gls{s-wcgbt} surveys. Given variable oceangraphic conditions north and south of Pt. Conception, variations of growth in the same species among areas my occur.  Ideally a full area specific growth curve would be estimated for both sexes ($k$, $L_{\infty}$, $L1$, and $L2$ within SS), however, given limitation in ageing capacity developing an area specific $L_{\infty}$ was prioritized. The Cooperative Ageing Program (CAP) selected a subsample of larger (greater then 35 cm) of `r spp` observed by both surveys (Figure \ref{fig:south-len-at-age}). Figure \ref{fig:len-age-comp} shows the length-at-age observed in Oregon and Washington compared to sub-sampled larger fish from south of Pt. Conception.  Since only lareger fish were aged from the southern area, no conclusions about the inflection of growth between areas can be made. The length-at-age curve fixed with the base model used the $k$ and $L1$ parameters estimated from fish of the coast of Oregon and Washington and the $L_{\infty}$ estimated from from the `r hkl` and \Gls{s-wcgbt} survey samples.

Figure \ref{fig:len-age-ss} shows sex-specific growth curve assumed in the base model. The parameters were fixed at the following values:
...
The coefficient of variation (CV) around young and old fish was fixed at a value of 0.10. 
